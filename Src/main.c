...
int main(void)
{
	...
	while (1)
	{
		if (bldc_timer - bldc_timer_prev > 16 * DELAY_IN_MAIN_LOOP)
		{ // 1 ms = 16 ticks bldc_timer
...
// ####### VARIANT_HOVERCAR #######
			...
			{ // Only use use implementation below if pedals are in use (ADC input)
				if (speedAvgAbs < 60)
			...
			{ // When Throttle is negative, it acts as brake. This condition is to make sure it goes to 0 as we reach standstill (to avoid Reverse driving)
				if (speedAvg > 0)
				{ // Make sure the braking is opposite to the direction of motion
					input2[inIdx].cmd = (int16_t)((input2[inIdx].cmd * speedBlend) >> 15);
				}
				else
				{
...
// ####### DEBUG SERIAL OUT #######
		...
		{ // Send data periodically every 125 ms
...
// ####### FEEDBACK SERIAL OUT #######
		...
		{ // 3 beeps (low pitch): Serial timeout
		...
		{ // 1 beep fast (medium pitch): Low bat 1
			beepCount(&buzzer, 0, 10, 6);
		}
		else if (BAT_LVL2_ENABLE && batVoltage < BAT_LVL2)
		{ // 1 beep slow (medium pitch): Low bat 2
